 ...
 * Returns all ruchettes (hives with purpose "Ruchette") in the apiary,
 * with their current OPEN nuc_cycle (if any).
 * [
 *   {
 *     ruchette: { hive_id, hive_code, public_key, hive_type, frame_capacity },
 *     cycle: { ...open cycle or null }
 *   }
 * ]
 */
router.get("/by-apiary/:apiaryId/list", authenticateUser, async (req, res) => {
   const apiary_id = Number(req.params.apiaryId);
   if (!apiary_id) return res.status(400).json({ error: "apiaryId is required" });

   try {
      // 1) Get all ruchettes in this apiary
      ...
      // 2) Get OPEN cycles for these ruchettes
