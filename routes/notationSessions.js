      ...
      .insert([
        {
          ...
          ended_at: startedAt, // will be updated on finish
          user_id,
        },
      ])
      .select()
...
router.post("/:id/entries", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const newAnswers = merge && existing?.answers
...
router.get("/:id", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const { data: catalog } = selectedKeys.length
      ? await supabase.from("notations_catalog").select("*").in("key", selectedKeys).order("sort_order", { ascending: true })
      : { data: [] };
