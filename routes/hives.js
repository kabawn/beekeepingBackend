...
// ✅ GET /hives (all user's hives) — MUST BE BEFORE "/:id"
// ✅ GET /hives (all user's hives) — MUST BE BEFORE "/:id"
router.get("/", authenticateUser, async (req, res) => {
   ...
   try {
      ...
      const cleaned = (data || []).map(({ apiaries, ...h }) => ({
         ...h,
         apiary_name: apiaries?.apiary_name ?? null,
      }));

      return res.status(200).json({ hives: cleaned });
   } catch (err) {
